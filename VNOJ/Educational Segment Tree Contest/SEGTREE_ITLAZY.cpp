...
struct RangeSetAddMinSumOps {
    ...
    static S op(S l, S r) { return S { l.sum + r.sum, max(l.min, r.min), l.sz + r.sz }; }
    static S e() { return S {0LL, INT_MIN, 0}; }
...
void run_case() {
	...
	vector<RangeSetAddMinSumOps::S> v(n);
	for (int i = 0; i < n; i++) {
		cin >> a[i];
		v[i].min = a[i];
		v[i].sum = 0;
	...
	LazySegTree<RangeSetAddMinSumOps::S, RangeSetAddMinSumOps::op, RangeSetAddMinSumOps::e, RangeSetAddMinSumOps::F, RangeSetAddMinSumOps::mapping, RangeSetAddMinSumOps::composition, RangeSetAddMinSumOps::id> st(v);
